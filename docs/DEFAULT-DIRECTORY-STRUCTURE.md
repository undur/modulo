...
* Access log location: `access-logs/` at top level won over `stacklogs/access/` (mixes log kinds with different owners and lifecycles) and a generic `logs/{stack,access}/` (reopens the stacklogs naming through hierarchy; its virtue — one answer to "where are the logs" — noted as the alternative).
